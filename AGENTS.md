# Local S3-compatible dev server that maps a plain directory, with a built-in UI (`s3-dir-dev-server`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://s3-dir-dev-server.sociobot.in`. Artifact class: `cli`.
